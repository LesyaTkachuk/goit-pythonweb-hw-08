...
from src.database.models import Contact, Group
...
class ContactRepository:
    ...
    async def get_contacts(
        self, skip: int, limit: int, query: str | None
    ) -> List[Contact]:
    ...
    async def create_contact(self, body: ContactModel, groups: List[Group]) -> Contact:
        contact = Contact(
            **body.model_dump(exclude={"groups"}, exclude_unset=True), groups=groups
        )
        self.db.add(contact)
    ...
    async def update_contact(
        self, contact_id: int, body: ContactUpdate, groups: List[Group]
    ) -> Contact | None:
        contact = await self.get_contact_by_id(contact_id)
        if contact:
